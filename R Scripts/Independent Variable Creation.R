...
setwd("G:/My Drive/10_Carlson/05_Research/12_Governance and Incidents")
...
# Load data from Will 
panel_wd <- as.data.table(readRDS("./02_Data/secdata/data_essay2_robustness_v2.rds")) 
panel_wd <- as.data.table(readRDS("./02_Data/secdata/rrpanel_comp_fs_fortune_cdp.rds"))
...
load_BoardEx <- function(link_tbl = NULL){
  ...
  if(TRUE){
    dt_CSO <- dt_TMT[,.(CSOs=max(CSO),
                        CSOs_M=max(CSO_M),
                        CSOs_F=max(CSO_F),
                        CSOs_SD=max(CSO_SD),
                        CSOs_ED=max(CSO_ED),
                        CSOs_SM=max(CSO_SM),
                        CSOs_Board=max(CSO_Board)),.(CompanyID,Year)]
    # sum over all boards per board member
    dt_TMT <- merge(dt_TMT, dt_CSO, by=c("CompanyID","Year"))
    dt_TMT <- merge(dt_TMT, dt_committee, by.x=c("CompanyID","Year"),by.y=c("BoardID","Year"),
                    all.x = TRUE)
    ...
    dt_TMT <- merge(dt_TMT, link_tbl[,.(companyid, gvkey ,PERMCO)], by.x=c('CompanyID'),
  ...
  dt_BoardEx <- dt_TMT[,.(CSO=max(CSOs), 
                          #CSO_M=max(CSOs_M), 
                          #CSO_F=max(CSOs_F), 
                          #CSO_SD=max(CSOs_SD), 
                          #CSO_ED=max(CSOs_ED), 
...
load_Refinitive <- function(){
  ...
  dt_screening <- dt_screening %>%
    .[fieldname %in% c("PolicyExecutiveCompensationESGPerformance",
                       "SupplierESGTraining")] %>%
    .[cusip != ""] %>%
...
load_board_Refinitive <- function(){
  ...
  dt_board <- dt_board %>%
    .[,Man := case_when(IsBMember==1 & BM_Gender=="Man" ~ 1,
  ...
  dt_board <- dt_board %>%
    .[,Compensation := case_when(IsBMember==1 ~ Board_Total_Compensation,
                                 IsBMember==0 ~ Executive_Total_Compensation)]
  
  
  dt_board <- dt_board %>%
    .[,CSO := case_when(Role %like% "ustaina" ~ 1,
                             Role %like% "nviron" ~ 1,
                             Role %like% "ocial" ~ 1,
                             Role %like% "esponsibility" ~ 1,
                             Role %like% "CSR" ~ 1,
                             TRUE ~ 0)]%>%
    .[,CSO_F := case_when(CSO == 1 & Man==0 ~ 1,
                          TRUE ~ 0)]%>%
    .[,CSO_M := case_when(CSO == 1 & Man==1 ~ 1,
                          TRUE ~ 0)]%>%
    .[,COO_Role := case_when(Role %like% "upply" ~ 1,
