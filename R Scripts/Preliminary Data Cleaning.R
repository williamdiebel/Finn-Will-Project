...
# 7515
panel_wd %>%
  filter(headquarter_country %in% c(
...
panel_joined_complete_cases %>%
  ...
  group_by(conm, gvkey, headquarter_country) %>%
  summarise()
...
# Follow-up (FP 06/25):
# The non-North American headquartered firms that are included seem to be firms that trade on U.S.
# stock exchanges but are headquarters abroad. I confirmed this patter for 5 firms from
# different countries.
...
# CSO firms
CSO_firms_cross_sectional <- data.frame(
  conm = CSO_firms,
...
# Non-CSO firms
non_CSO_firms_cross_sectional <- data.frame(
  conm = non_CSO_firms,
  stringsAsFactors = FALSE
)
...
CSO_firms_cross_sectional %>%
  group_by(headquarter_country) %>%
  count() %>%
  arrange(desc(n))
non_CSO_firms_cross_sectional %>%
  group_by(headquarter_country) %>%
  count() %>%
  arrange(desc(n))
...
panel_intersection <- panel_intersection %>%
  ...
  mutate(treatment_start = ifelse(any(CSO == 1), # identifies whether it is "treated", i.e., CSO == 1
    min(year[CSO == 1], na.rm = TRUE), NA
  )) %>% # and, if so, assigns the initial treatment period (min year)
  ungroup()

panel_intersection %>%
  select(conm, year, CSO, treatment_start) %>%
  filter(conm == "POLYMET MINING CORP") # visually inspecting a CSO firm to ensure the above worked as intended
# looks good
...
panel_intersection %>%
  select(conm, year, CSO, treatment_start, pre_treatment) %>%
  filter(conm == "POLYMET MINING CORP") # visually inspecting a CSO firm to ensure the above worked as intended
# looks good
...
panel_matched %>%
  select(conm, year, total_incident_count, environmental_incident_count) %>%
  arrange(desc(total_incident_count)) %>%
  view()
...
model <- feols(
  environmental_incident_count ~ cdp_sc_member + log(at_gbp_winsorized_1) + supplier_count + prop_suppliers_cdp_sc +
    +roa_winsorized_1
    + CSO |
    reprisk_strata + year, # Firm and time fixed effects
  data = panel_matched,
...
model <- feols(
  environmental_incident_count ~ cdp_sc_member + CSO * log(at_gbp_winsorized_1) + supplier_count + prop_suppliers_cdp_sc +
    +roa_winsorized_1 |
    reprisk_strata + year, # Firm and time fixed effects
  data = panel_matched,
  cluster = "reprisk_strata",
...
model <- feols(
  social_incident_count ~ cdp_sc_member + log(at_gbp_winsorized_1) + supplier_count + prop_suppliers_cdp_sc +
    +roa_winsorized_1
    + CSO |
    reprisk_strata + year, # Firm and time fixed effects
  data = panel_matched,
...
model <- feols(
  climate_incident_count ~ cdp_sc_member + log(at_gbp_winsorized_1) + supplier_count + prop_suppliers_cdp_sc +
    +roa_winsorized_1
    + CSO |
    reprisk_strata + year, # Firm and time fixed effects
  data = panel_matched,
...
mfg <- c(
  "Materials", "Automobiles & Components", "Capital Goods",
...
model <- feols(
  environmental_incident_count ~ log(at_gbp_winsorized_1)
  + roa_winsorized_1
    + CSO |
    reprisk_strata + year, # Firm and time fixed effects
  data = mfg_subset,
...
model <- feols(
  social_incident_count ~ log(at_gbp_winsorized_1)
  + roa_winsorized_1
    + CSO |
    reprisk_strata + year, # Firm and time fixed effects
  data = mfg_subset,
...
model <- feols(
  climate_incident_count ~ log(at_gbp_winsorized_1)
  + roa_winsorized_1
    + CSO |
    reprisk_strata + year, # Firm and time fixed effects
  data = mfg_subset,
...
pre_and_post_dvs <- data.frame(
  ...
  pre_mean = c(
    mean(treated_pre$environmental_incident_count, na.rm = TRUE),
    mean(treated_pre$social_incident_count, na.rm = TRUE),
    mean(treated_pre$climate_incident_count, na.rm = TRUE)
  ),
  post_mean = c(
    mean(treated_post$environmental_incident_count, na.rm = TRUE),
